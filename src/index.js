...
module.exports = (
  { markdownAST },
  {
    ...
    isIconAfterParagraph = false,
    elements = null,
  }
) => {
  slugs.reset()

  // Values for id are assigned based on the raw <p> count
  var counter = 1;
  ...
  visit(markdownAST, `paragraph`, node => {
    ...
    if (icon !== false) {
      ...
      const method = isIconAfterParagraph ? `push` : `unshift`
      node.children[method]({
        ...
        data: {
          hProperties: {
            ...
            class: `${className} ${isIconAfterParagraph ? `after` : `before`}`,
          },
          hChildren: [
            {
              type: `raw`,
